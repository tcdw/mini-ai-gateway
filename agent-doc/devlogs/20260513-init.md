...
```
ai-gateway/
├── .env                    # 密钥（GATEWAY_API_KEY + Provider keys）
├── .env.example            # （待创建）
├── config.toml             # Provider 定义 + 模型路由表
├── models-meta.json        # 同步脚本生成的模型元数据
├── index.ts                # 网关主程序
├── scripts/
│   └── sync-models.ts      # 模型元数据同步脚本
├── package.json
└── tsconfig.json
```
...
authHeader = "Bearer"
keyEnvVar = "NEWPROVIDER_API_KEY"

[models."example/model-name"]

[[models."example/model-name".providers]]
name = "newprovider"
weight = 5
remap = "their-internal-model-id"
```

然后 `.env` 加上 `NEWPROVIDER_API_KEY=xxx`，跑一次 `bun run scripts/sync-models.ts`。
